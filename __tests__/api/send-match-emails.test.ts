...
const TEST_MONTH = "2099-03";
const TEST_MONTH_DATE = "2099-03-01";
...
async function seedCommittedRound(): Promise<string> {
  ...
  const { data, error } = await supabase
    ...
    .select("id")
    .single();
  if (error || !data) throw new Error(`seedCommittedRound failed: ${error?.message}`);
  return data.id as string;
}

async function seedMatch(member1Id: string, member2Id: string): Promise<string> {
  const supabase = createTestSupabase();
...
async function seedParticipation(memberId: string, topicId: string): Promise<void> {
  const supabase = createTestSupabase();
